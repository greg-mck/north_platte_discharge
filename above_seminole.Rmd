...
1. Download daily discharge data for North PLatte River river in Seminole, WY. from 1990-2023
...
discharge_data <- readNWISdv(siteNumber = "06630000",
                          parameterCd = "00060", 
...
p <- ggplot(discharge_data, aes(x = date, y = q_cfs)) +
  ...
  ggtitle(label ="North Platte Daily discharge", 
          subtitle = "Seminole, WY. 1990 - 2023") +
  ylab('Q (cfs)') + 
  xlab('date')
...
p <- ggplot(lastThree, aes(x = date, y = q_cfs)) +
  geom_point() +
  ggtitle(label ="North Platte Daily discharge", 
          subtitle = "Seminole, WY. 2018 - 2022") +
...
p <- ggplot(summertime_mean, aes(x = year, y = summerTimeMean_q)) +
  ...
  ggtitle(label ="North Platte Summer time average discharge", 
          subtitle = "Seminole, WY. June 21 - Sept 21") +
